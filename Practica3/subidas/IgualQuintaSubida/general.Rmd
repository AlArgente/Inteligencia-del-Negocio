...
```{r}
set.seed(371)
objControl <- trainControl(method = "cv", number = 5, allowParallel = T)
gbmFit <- gbm(SalePrice ~., data = Training, shrinkage = 0.1, interaction.depth=3,
...
```{r}
rmse(Validation$SalePrice, (pred + predsp1)/2)
```

Ensamble de los algoritmos utilizados.
```{r}
preds.ensamble <- 0.2 * predXGB + 0.3 * predsGBM + 0.5 * preds.lasso
