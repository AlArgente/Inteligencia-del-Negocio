...
```{r}
set.seed(371)
gbmFit <- gbm(SalePrice ~., data = Training, shrinkage = 0.015, interaction.depth=3,
...
```{r}
set.seed(371)
gbmFited <- gbm(SalePrice ~., data = training, shrinkage = 0.015, interaction.depth=3,
              bag.fraction = 0.5308, n.minobsinnode = 3,
              cv.folds = 5, verbose=FALSE, distribution = "gaussian", n.trees = 2000)
...
xgbFit <- xgboost(data = dtrain, nfold = 5, labels = as.matrix(Training$SalePrice),
                  ...
                  nthread = 4, eta = 0.01, gamma = 0.01, max_depth = 3, min_child_weight = 1.782,
...
```{r}
preds.ensamble <- 0.55 * predXGB + 0 * predsGBM + 0.45 * as.numeric(preds.lasso)
ensamble.df<- data.frame(Id = test_ID, SalePrice = preds.ensamble)
write.csv(ensamble.df, "Xgb3GBM4Lasso3Ensamble.csv", row.names = FALSE)
```