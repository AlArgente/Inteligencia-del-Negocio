...
```{r}
rmse(Validation$SalePrice, (0.5 * pred + 0.2 * predsp1 + 0.3 * predsp2))
```

Ensamble de los algoritmos utilizados.
```{r}
preds.ensamble <- 0.4 * predXGB + 0.3 * as.numeric(preds.lasso) + 0.3 * predsGBM
ensamble.df<- data.frame(Id = test_ID, SalePrice = preds.ensamble)
write.csv(ensamble.df, "EnsambleXgb4Lass3GBM3.csv", row.names = FALSE)
```