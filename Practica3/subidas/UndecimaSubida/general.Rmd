...
```{r}
rmse(Validation$SalePrice, (0.5 * pred + 0.25 * predsp1 + 0.25 * predsp2))
```

Ensamble de los algoritmos utilizados.
```{r}
preds.ensamble <- 0.5 * predXGB + 0.25 * as.numeric(preds.lasso) + 0.25 * predsGBM
ensamble.df<- data.frame(Id = test_ID, SalePrice = preds.ensamble)
write.csv(ensamble.df, "EnsambleXgb5Lass25GBM25.csv", row.names = FALSE)
```